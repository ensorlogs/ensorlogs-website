        ...
        <div class="nav max-md:py-3 py-5 px-4 lg:px-6 xl:px-8 flex items-center justify-between bg-white dark:bg-[#1B1C1C] rounded-xl shadow-lg shadow-black/5">
                ...
                <span class="flex flex-col justify-center min-w-0 leading-none">
                    ...
                    <span class="mt-1 text-[11px] md:text-xs font-medium text-darkGray dark:text-pastelGrey tracking-tight leading-snug">
                        ...
                        if (function_exists('ensorlogs_header_flag_html')) {
                            ...
                            echo ensorlogs_header_flag_html();
                        }
                        ?>
                    </span>
</span>
            ...
            <div class="main-menu *:transition-all *:text-darkGray *:inline-flex *:items-center dark:*:text-pastelGrey *:border *:border-transparent *:duration-300 *:px-6 *:py-2 *:leading-normal *:rounded-4xl *:font-semibold max-lg:hidden">
                <a href="<?php echo esc_url(function_exists('ensorlogs_lang_url') ? ensorlogs_lang_url('/') : home_url('/')); ?>" class="ensor-nav-volver dark:[&.active]:text-white [&.active]:border-nobelGray dark:[&.active]:border-white/20 hover:text-smokeGray dark:hover:text-white">
                    <?php echo esc_html(function_exists('ensorlogs_t') ? ensorlogs_t('Volver', 'Home') : 'Volver'); ?>
                </a>
                <a href="<?php echo esc_url(function_exists('ensorlogs_lang_url') ? ensorlogs_lang_url('/blog/') : home_url('/blog/')); ?>" class="dark:[&.active]:text-white [&.active]:border-nobelGray dark:[&.active]:border-white/20 hover:text-smokeGray dark:hover:text-white">
                    <?php echo esc_html(function_exists('ensorlogs_t') ? ensorlogs_t('Hablemos de…', "Let's talk…") : 'Hablemos de…'); ?>
                </a>
                <a href="<?php echo esc_url(function_exists('ensorlogs_lang_url') ? ensorlogs_lang_url('/about/') : home_url('/about/')); ?>" class="dark:[&.active]:text-white [&.active]:border-nobelGray dark:[&.active]:border-white/20 hover:text-smokeGray dark:hover:text-white">
                    <?php echo esc_html(function_exists('ensorlogs_t') ? ensorlogs_t('Sobre mi', 'About me') : 'Sobre mi'); ?>
                </a>
                <a href="<?php echo esc_url(function_exists('ensorlogs_lang_url') ? ensorlogs_lang_url('/projects/') : home_url('/projects/')); ?>" class="dark:[&.active]:text-white [&.active]:border-nobelGray dark:[&.active]:border-white/20 hover:text-smokeGray dark:hover:text-white">
                    <?php echo esc_html(function_exists('ensorlogs_t') ? ensorlogs_t('Proyectos', 'Projects') : 'Proyectos'); ?>
                </a>
                <a href="<?php echo esc_url(function_exists('ensorlogs_lang_url') ? ensorlogs_lang_url('/services/') : home_url('/services/')); ?>" class="dark:[&.active]:text-white [&.active]:border-nobelGray dark:[&.active]:border-white/20 hover:text-smokeGray dark:hover:text-white">
                    <?php echo esc_html(function_exists('ensorlogs_t') ? ensorlogs_t('¿Cómo puedo ayudarte?', 'How can I help?') : '¿Cómo puedo ayudarte?'); ?>
                </a>
            </div>
            <div class="button-groups flex items-center gap-4 *:border *:border-flasWhite *:dark:border-flasBlack *:bg-gradient-to-b *:from-milkWhite *:to-seashell dark:*:from-metalBlack dark:*:to-oilBlack">
                <button class="btn_theme_switch max-lg:hidden btn-dark w-13 h-13 max-md:w-12 max-md:h-12 relative rounded-full group *:w-5 *:h-5 *:max-md:w-6 *:max-md:h-6 *:absolute *:top-1/2 *:left-1/2 *:-translate-x-1/2 *:-translate-y-1/2 " aria-label="Dark - Light Switch">
...
<div class="mobile_menu fixed transition-all duration-300 top-0 -left-96 h-full bg-white dark:bg-powerBlack z-999 p-6 w-80 shadow-lg [&.is-menu-open]:left-0 flex flex-col lg:hidden">
    ...
    <div class="my-12 h-calc(100vh_-_16rem) overflow-y-scroll space-y-2 *:text-powerBlack *:bg-slate-50 dark:*:bg-metalBlack dark:*:text-white *:flex *:items-center *:justify-between *:py-2.5 *:px-4 *:rounded-md *:text-regular">
        <a href="<?php echo esc_url(function_exists('ensorlogs_lang_url') ? ensorlogs_lang_url('/') : home_url('/')); ?>" class="ensor-nav-volver [&.active]:bg-powerBlack dark:[&.active]:bg-black [&.active]:text-white dark:[&.active]:text-white">
            <?php echo esc_html(function_exists('ensorlogs_t') ? ensorlogs_t('Volver', 'Home') : 'Volver'); ?>
        </a>
        <a href="<?php echo esc_url(function_exists('ensorlogs_lang_url') ? ensorlogs_lang_url('/blog/') : home_url('/blog/')); ?>" class="[&.active]:bg-powerBlack dark:[&.active]:bg-black [&.active]:text-white dark:[&.active]:text-white">
            <?php echo esc_html(function_exists('ensorlogs_t') ? ensorlogs_t('Hablemos de…', "Let's talk…") : 'Hablemos de…'); ?>
        </a>
        <a href="<?php echo esc_url(function_exists('ensorlogs_lang_url') ? ensorlogs_lang_url('/about/') : home_url('/about/')); ?>" class="[&.active]:bg-powerBlack dark:[&.active]:bg-black [&.active]:text-white dark:[&.active]:text-white">
            <?php echo esc_html(function_exists('ensorlogs_t') ? ensorlogs_t('Sobre mi', 'About me') : 'Sobre mi'); ?>
        </a>
        <a href="<?php echo esc_url(function_exists('ensorlogs_lang_url') ? ensorlogs_lang_url('/projects/') : home_url('/projects/')); ?>" class="[&.active]:bg-powerBlack dark:[&.active]:bg-black [&.active]:text-white dark:[&.active]:text-white">
            <?php echo esc_html(function_exists('ensorlogs_t') ? ensorlogs_t('Proyectos', 'Projects') : 'Proyectos'); ?>
        </a>
        <a href="<?php echo esc_url(function_exists('ensorlogs_lang_url') ? ensorlogs_lang_url('/services/') : home_url('/services/')); ?>" class="[&.active]:bg-powerBlack dark:[&.active]:bg-black [&.active]:text-white dark:[&.active]:text-white">
            <?php echo esc_html(function_exists('ensorlogs_t') ? ensorlogs_t('¿Cómo puedo ayudarte?', 'How can I help?') : '¿Cómo puedo ayudarte?'); ?>
        </a>
        <div class="ensor-mobile-theme-toggle flex items-center justify-between gap-3 py-2.5 px-4 rounded-md bg-slate-50 dark:bg-metalBlack text-powerBlack dark:text-white">
            <span class="font-medium"><?php echo esc_html(function_exists('ensorlogs_t') ? ensorlogs_t('Claro / oscuro', 'Light / dark') : __('Claro / oscuro', 'ensorlogs')); ?></span>
            <button type="button" class="btn_theme_switch w-13 h-13 relative rounded-full group border border-flasWhite dark:border-flasBlack bg-gradient-to-b from-milkWhite to-seashell dark:from-metalBlack dark:to-oilBlack *:w-5 *:h-5 *:absolute *:top-1/2 *:left-1/2 *:-translate-x-1/2 *:-translate-y-1/2 " aria-label="<?php esc_attr_e('Dark - Light Switch', 'ensorlogs'); ?>">
                <svg class="sun group-[&.btn-light]:opacity-0" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M11 17.5C14.5899 17.5 17.5 14.5899 17.5 11C17.5 7.41015 14.5899 4.5 11 4.5C7.41015 4.5 4.5 7.41015 4.5 11C4.5 14.5899 7.41015 17.5 11 17.5Z" stroke="#CDD0DA" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M18.14 18.14L18.01 18.01M18.01 3.99L18.14 3.86L18.01 3.99ZM3.86 18.14L3.99 18.01L3.86 18.14ZM11 1.08V1V1.08ZM11 21V20.92V21ZM1.08 11H1H1.08ZM21 11H20.92H21ZM3.99 3.99L3.86 3.86L3.99 3.99Z" stroke="#CDD0DA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
    ...
    <div class="mt-auto">
        <div class="cta_button text-center space-y-1">
            <p class="font-medium"><?php echo esc_html(function_exists('ensorlogs_t') ? ensorlogs_t('¿Hablamos?', 'Shall we talk?') : __('¿Hablamos?', 'ensorlogs')); ?></p>
            <a href="<?php echo esc_url(function_exists('ensorlogs_lang_url') ? ensorlogs_lang_url('/contact/') : home_url('/contact/')); ?>" class="bg-darkGray dark:bg-metalBlack inline-flex text-white font-semibold py-3 px-16 leading-1.75 rounded-md">
                <?php echo esc_html(function_exists('ensorlogs_t') ? ensorlogs_t('Contactar', 'Contact') : 'Contactar'); ?>
            </a>
        </div>
    </div>
</div>
