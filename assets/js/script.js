...
'use strict';
    ...
    (function preloaderRandomMessage() {
        ...
        var messagesEs = [
            ...
            'chmod +x ./vida && ./vida',
            'Ejecutando SELECT * FROM paciencia…',
            'Esperando que termine el VLOOKUP…',
            'Normalizando datos hasta la tercera forma…',
            'Limpiando nulls, outliers y excusas',
            'Cruzando JOINs que nadie pidió',
            'Convirtiendo Excel en base de datos (otra vez)',
            'Un analista sin café es un LEFT JOIN sin ON',
            '#N/A — sigue cargando igual',
            'Agregando métricas que nadie revisará',
            'WHERE duda IS NULL AND datos IS NOT NULL',
            'Indexando la paciencia del analista…',
            'ETL: Extract, Transform, Latte',
            'COUNT(*) de los segundos que faltan…',
            'Backup de la hoja "final_v2_definitiva"',
            'Haciendo GROUP BY de mil excusas',
            'El CSV pesa más que mi confianza en él',
            'Optimizando query… o intentándolo',
            'Ese gráfico no es bonito hasta que cuadra',
            'Pivot table activada. Mesas pivotantes, no.',
            'Migrando de Excel a SQL… con lágrimas',
            'El dashboard no miente, pero a veces omite detalles',
            'Deduplicando filas duplicadas duplicadas',
            'COMMIT pendiente de aprobación del negocio'
        ];
        var messagesEn = [
            ...
            'chmod +x ./life && ./life',
            'Running SELECT * FROM patience…',
            'Waiting for VLOOKUP to finish…',
            'Normalizing data to third normal form…',
            'Cleaning nulls, outliers, and excuses',
            'Joining tables nobody asked for',
            'Turning Excel into a database (again)',
            'An analyst without coffee is a LEFT JOIN with no ON',
            '#N/A — still loading anyway',
            'Aggregating metrics nobody will read',
            'WHERE doubt IS NULL AND data IS NOT NULL',
            'Indexing the analyst\'s patience…',
            'ETL: Extract, Transform, Latte',
            'COUNT(*) of seconds remaining…',
            'Backing up sheet "final_v2_definitive"',
            'GROUP BY of a thousand excuses',
            'This CSV weighs more than my trust in it',
            'Optimizing query… or trying to',
            'That chart isn\'t pretty until the numbers match',
            'Pivot table on. Actual pivot tables, off.',
            'Migrating from Excel to SQL… with tears',
            'The dashboard never lies — it just omits details',
            'Deduplicating duplicate duplicate rows',
            'COMMIT pending business approval'
        ];
